...
import { Image, Folder, Link, RefreshCw, Sun, Moon, ArrowLeft, X, Copy, Grid3x3, List, Search, Calendar } from 'lucide-vue-next'
  ...
  <div class="min-h-screen container" :class="themeStore.isDark ? 'bg-[var(--bg-primary)]' : 'bg-gray-50'">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
        ...
        <div class="relative flex-1">
          ...
          <input v-model="search" type="text" placeholder="搜索文件名..."
            @keyup.enter="handleSearch"
            class="w-full pl-10 pr-4 py-2.5 rounded-xl border transition-all"
        ...
        <div class="flex items-center gap-2">
          <div class="flex items-center gap-1 rounded-lg border overflow-hidden"
            :class="themeStore.isDark ? 'border-[var(--border)]' : 'border-gray-200'">
            <button v-for="preset in filterPresets" :key="preset.value"
          ...
          <el-popover v-model:visible="showDatePicker" trigger="click" placement="bottom" :width="300">
            <template #reference>
              <button class="p-2.5 rounded-xl border transition-all"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div v-else-if="files.length === 0" class="text-center py-20 sm:py-32">
        ...
        <p class="text-base sm:text-lg mb-3 sm:mb-4" :class="themeStore.isDark ? 'text-gray-400' : 'text-gray-500'">暂无图片</p>
          ...
          <table class="w-full min-w-[500px]">
            ...
            <tbody>
              <tr v-for="file in files" :key="file.id"
                ...
                :class="themeStore.isDark ? 'border-[var(--border)]' : 'border-gray-100'" @click="openPreview(file.url)">
                <td class="p-3 sm:p-4">
                ...
                <td class="p-3 sm:p-4 text-sm hidden md:table-cell" :class="themeStore.isDark ? 'text-gray-400' : 'text-gray-500'">
